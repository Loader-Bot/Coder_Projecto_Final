...
def editar_post(request, pk):
    page = get_object_or_404(NewsPost, pk=pk, author=request.user)
    if request.method == 'POST':
    ...
    return render(request, 'editar_post.html', {'form': form, 'pk': pk})
...
def borrar_post(request, pk):
    ...
    if request.method == 'POST':
        page.delete()
        return redirect('index')
    return render(request, 'borrar_post.html', {'page': page})